...
# NixOS bootstrap: provide runtime dependencies through the modern `nix shell` CLI.
# This keeps the upstream script behaviour on other distros, but avoids apt/yum/pacman
# auto-install attempts on NixOS.
IPQUALITY_ORIGINAL_ARGS=("$@")
IPQUALITY_NIX_PACKAGES=(
  nixpkgs#bash
  nixpkgs#coreutils
  nixpkgs#curl
  nixpkgs#jq
...
ipquality_is_nixos(){
  ...
  . /etc/os-release
  [ "${ID:-}" = "nixos" ] || [ "${ID_LIKE:-}" = "nixos" ]
}

ipquality_missing_runtime_deps(){
  local cmd
  for cmd in bash curl jq bc nc dig nslookup ip timeout awk grep sed sort head cut tr od gunzip; do
    command -v "$cmd" >/dev/null 2>&1 || return 0
  done
  return 1
}

ipquality_reexec_with_nix_shell(){
  [ "${IPQUALITY_IN_NIX_SHELL:-0}" = "1" ] && return 0
  ipquality_is_nixos || return 0
  ipquality_missing_runtime_deps || return 0

  if ! command -v nix >/dev/null 2>&1; then
    echo "ERROR: NixOS detected, but the 'nix' command is not available in PATH." >&2
    echo "Please enable Nix first, or run this script inside a shell containing: curl jq bc netcat-openbsd bind iproute2." >&2
    exit 1
  fi
  ...
  if [ -r "$script_path" ]; then
    case "$script_path" in
      /dev/fd/*|/proc/*/fd/*)
        local tmp_script="${TMPDIR:-/tmp}/ipquality-nixos-$$.sh"
        cp "$script_path" "$tmp_script" || exit 1
        chmod +x "$tmp_script" 2>/dev/null || true
        script_path="$tmp_script"
        ;;
    esac
  else
    echo "ERROR: Cannot re-enter nix shell because the script path is not readable: $script_path" >&2
    echo "Please save the script to a file, then run it again, for example: bash ./ipquality-nixos.sh" >&2
    exit 1
  fi

  echo "NixOS detected; missing runtime dependencies. Re-running through: nix shell ..." >&2
  exec env IPQUALITY_IN_NIX_SHELL=1 \
    nix --extra-experimental-features "nix-command flakes" shell "${IPQUALITY_NIX_PACKAGES[@]}" \
    -c bash "$script_path" "${IPQUALITY_ORIGINAL_ARGS[@]}"
}
...
case $ID in
nixos)ipquality_reexec_with_nix_shell "${IPQUALITY_ORIGINAL_ARGS[@]}"
echo "ERROR: NixOS detected, but dependencies are still missing after nix shell re-entry."
echo "Try manually: nix --extra-experimental-features \"nix-command flakes\" shell ${IPQUALITY_NIX_PACKAGES[*]} -c bash $0 ${IPQUALITY_ORIGINAL_ARGS[*]}"
exit 1
;;
